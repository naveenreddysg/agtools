...
blueprint = Blueprint('vegetables', __name__)

@blueprint.route("/admin_vegetables_meta")
@login_required
def vegetables_list():
    """list all available vegetables and respective permission.
    :return: dict: users dict items with available permissions
    """
    vegetables_meta = client_raw.marketnews.distinct("Commodity",{"commodity_type":"vegetables"})

    pipe = [{'$group' : {'_id': {'Name':"$Commodity",'Type':"$commodity_type"},
				"Terminal_Market" :  {'$sum' : {'$cond' : { 'if': { '$eq': ["$report_type", "Terminal Market"]}, 'then': 1, 'else': 0}
					}},
				"Shipping_Point" :  {'$sum' : {'$cond' : { 'if': { '$eq': ["$report_type", "Shipping Point"]}, 'then': 1, 'else': 0}
					}},
                "Movement" :  {'$sum' : {'$cond' : { 'if': { '$eq': ["$report_type", "Movement"]}, 'then': 1, 'else': 0}
					}}
			} }	]
    vegetables_count = client_raw.marketnews.aggregate(pipeline=pipe)

    return render_template('admin_vegetables_meta.html', result_items=vegetables_count,
                           total_vegetables=len(vegetables_meta))

@blueprint.route("/admin_vegetables_shipping", methods = ['GET','POST'])
@login_required
def vegetables_shipping_point():
    ...
    try:
        ...
        vegetables_shipping = client_raw.marketnews.find({"report_type": "Shipping Point", "commodity_type": "vegetables", "report_date": {'$gte': d, '$lt': f}})
        total_shipping = vegetables_shipping.count()
        fallback = ""
        records = [{'commodity': str(terminal['Commodity']),
                    ...
                    } for terminal in vegetables_shipping]
    except Exception as e:
        vegetables_shipping = client_raw.marketnews.find({'commodity_type': cts.COMMODITY_CLASS[1].lower(),
    ...
    #                                                          'commodity_class': cts.COMMODITY_CLASS[1]})
        total_shipping = client_raw.marketnews.find({"report_type":"Shipping Point","commodity_type":"vegetables"}).count()
    ...
    return render_template('admin_vegetables_shipping.html',
                           result_items=records,total_vegetables=total_shipping,FromDate=FromDate,ToDate=ToDate)
                           # total_vegetables=total_shipping['total_count']

@blueprint.route("/admin_vegetables_terminal", methods = ['GET','POST'])
@login_required
def vegetables_terminal_market():
    ...
    try:
        ...
        d = dttim.datetime(int(FromDate1[2]), int(FromDate1[0]), int(FromDate1[1]))
        f = dttim.datetime(int(ToDate1[2]), int(ToDate1[0]), int(ToDate1[1]))
        vegetables_terminal = client_raw.marketnews.find({"report_type": "Terminal Market", "commodity_type": "vegetables", "report_date": {'$gte': d, '$lt': f}})
        total_count = vegetables_terminal.count()
        fallback = ""
        records = [{'commodity': str(terminal['Commodity']),
                    ...
                    'storage': str(terminal.get('Storage', fallback)),
                    'appearance': str(terminal.get('Appearance', fallback))
                    } for terminal in vegetables_terminal]
    except Exception as e:
        vegetables_terminal = client_raw.marketnews.find({'commodity_type': cts.COMMODITY_CLASS[1].lower(),
    ...
    #                                                          'commodity_class': cts.COMMODITY_CLASS[1]})
        ...
        total_count = client_raw.marketnews.find({"report_type":"Terminal Market","commodity_type":"vegetables"}).count()
        fallback = ""
        records = [{'commodity': str(terminal['Commodity']),
                    'location': str(terminal.get('location', fallback)),
                    'origin': str(terminal.get('Origin', fallback)),
    ...
    return render_template('admin_vegetables_terminal.html',
                           result_items=records,total_vegetables=total_count,FromDate=FromDate,ToDate=ToDate)
                           # total_vegetables=total_count['total_count']
...
@blueprint.route("/admin_vegetables_volume", methods = ['GET','POST'])
@login_required
def vegetables_volume():
    ...
    try:
        ...
        d = dttim.datetime(int(FromDate1[2]), int(FromDate1[0]), int(FromDate1[1]))
        f = dttim.datetime(int(ToDate1[2]), int(ToDate1[0]), int(ToDate1[1]))
        vegetables_records = client_raw.marketnews.find({"report_type": "Movement", "commodity_type": "vegetables", "report_date": {'$gte': d, '$lt': f}})
        total_count = vegetables_records.count()
        fallback = ""
        fallbackint = 0
        package_count = 0
        records = [{'commodity': str(terminal['Commodity']),
                    ...
                    } for terminal in vegetables_records]
    except Exception as e:
        vegetables_records = client_raw.marketnews.find({'commodity_type': cts.COMMODITY_CLASS[1].lower(),
                                                  'report_type': cts.COMMODITY_REPORT_TYPE[2]}).sort("report_date", -1).limit(500)
    # total_count = client_raw.counts_commodity_class.find_one({'count_type': 'commodity_class',
    #                                                          'report_type': cts.COMMODITY_REPORT_TYPE[2],
    #                                                          'commodity_class': cts.COMMODITY_CLASS[1]})

        total_count = client_raw.marketnews.find({"report_type":"Movement","commodity_type":"vegetables"}).count()
        fallback = ""
        fallbackint = 0
        package_count = 0
        records = [{'commodity': str(terminal['Commodity']),
    ...
    return render_template('admin_vegetables_volume.html',
                           result_items=records,total_vegetables=total_count,FromDate=FromDate,ToDate=ToDate)
                           # total_vegetables=total_count['total_count']
